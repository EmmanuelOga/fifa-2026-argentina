...
function writeGuidance(s) {
  ...
  L.push('Fix these concrete items during the prose pass, on top of the 25–50% cut.\n');
...
function verify(now) {
  ...
  for (const lang of ['en', 'es']) {
    ...
    const cut = +wCut;
    if (cut < 25) console.log(c.yellow(`      note: ${lang.toUpperCase()} cut ${wCut}% — target is 25–50%.`));
    else if (cut > 55) console.log(c.yellow(`      note: ${lang.toUpperCase()} cut ${wCut}% — above the 50% target; check nothing load-bearing was lost.`));
  }
}
